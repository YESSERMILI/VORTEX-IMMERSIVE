...
export default function WorkDetailPage() {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      {/* Hero Section */}
      <section className="relative h-[80vh] flex items-end pb-16 md:pb-32 overflow-hidden border-b border-border">
        <Image 
        ...
        <div className="container mx-auto px-6 lg:px-12 max-w-[1400px] relative z-10">
          ...
          <div className="space-y-4 md:space-y-8 max-w-5xl">
            ...
            <h1 className="text-3xl md:text-6xl lg:text-7xl font-black font-headline tracking-tighter leading-[0.9] uppercase">
              {work.title}
            </h1>
            <p className="text-lg md:text-2xl text-muted-foreground leading-relaxed italic max-w-3xl border-l border-primary/20 pl-6 md:pl-10">
              {work.desc}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-16 lg:gap-24">
            {[
              ...
              <div key={i} className="space-y-2 md:space-y-4">
                ...
                <p className="text-xl md:text-2xl font-black font-headline uppercase tracking-tight">{item.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 md:py-40">
        <div className="container mx-auto px-6 lg:px-12 max-w-[1400px]">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 md:gap-24 lg:gap-32 items-start">
            <div className="lg:col-span-8 space-y-12 md:space-y-20">
              <div className="space-y-6 md:space-y-10">
                <h2 className="text-3xl md:text-4xl font-black font-headline uppercase tracking-tighter">The <span className="text-primary italic">Challenge</span></h2>
                <div className="space-y-6 md:space-y-8 text-base md:text-xl text-muted-foreground leading-relaxed max-w-4xl italic">
                  <p>
              ...
              <div className="relative aspect-video border border-border overflow-hidden bg-card group cursor-pointer shadow-2xl">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                   <div className="w-16 h-16 md:w-24 md:h-24 rounded-full border border-white/20 bg-white/5 backdrop-blur-xl flex items-center justify-center group-hover:scale-110 transition-transform duration-500">
                      <Play className="w-6 h-6 md:w-8 md:h-8 fill-white stroke-none ml-1" />
                   </div>
                </div>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-8 md:space-y-12">
              <div className="bg-card p-8 md:p-12 border border-border space-y-8 md:space-y-12 relative overflow-hidden">
                ...
                <ul className="space-y-8 md:space-y-10">
                  {[
                    ...
                    <li key={i} className="space-y-2 md:space-y-3">
                      <div className="flex items-center gap-3 text-muted-foreground">
                        {tech.icon}
                        <p className="text-[10px] uppercase font-black tracking-widest">{tech.label}</p>
                      </div>
                      <p className="text-sm md:text-base font-bold tracking-tight uppercase pl-7 border-l border-border">{tech.value}</p>
                    </li>
                  ))}
                </ul>
              </div>

              <div className="p-8 md:p-12 border border-border bg-primary/5 space-y-6 md:space-y-8">
                ...
                <div className="pt-4 border-t border-border">
                  <p className="text-[10px] md:text-xs font-black uppercase tracking-[0.3em]">— Lead Creative Director</p>
                  <p className="text-[9px] md:text-[10px] uppercase font-bold text-primary mt-1">{work.client}</p>
                </div>
              </div>
            </div>
          </div>
